...
void Game::Exit() { s_Instance->m_ShouldExit = true; }

// TODO(Pedro): Probably not the best for this, should change later
void PopulatePhysicsWorld(flecs::entity e, const Position &pos,
                          Rigidbody2D &rb) {
...
void PhysicsStep(flecs::iter it) {
  ...
  physicsWorld->Step(it.delta_time(), Physics2D::GetVelocityIterations(),
                     Physics2D::GetPositionIterations());
}
...
void Game::Run() {
  ...
  // Physics World Setup
  World::BindSystem<const Position, Rigidbody2D>(
      flecs::OnStart, "Populate physics world", PopulatePhysicsWorld);

  // Physics World Update
  World::BindSystem<const Position, Rigidbody2D>(
      flecs::PostUpdate, "Pre Physics Step", PrePhysicsStep);
  World::BindSystemNoQuery(flecs::PostUpdate, "Physics Step", PhysicsStep);
  World::BindSystem<Position, const Rigidbody2D>(
      flecs::PostUpdate, "Post Physics Step", PostPhysicsStep);

  // Render
  World::BindSystemNoQuery(flecs::PreStore, "Start Drawing", BeginDrawing);
  World::BindSystem<const Sprite2D, const Position, const Rotation,
                    const Scale>(flecs::PreStore, "Draw Sprite", DrawSprite);
...
void Game::CleanUp() { Physics2D::Shutdown(); }
} // namespace BladeEngine
