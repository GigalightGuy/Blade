...
void BladeEngine::Graphics::Vulkan::VulkanSwapchain::AddFramebuffers(VkDevice device, VkRenderPass renderPass)
{
    std::vector<VkFramebuffer> framebuffers(imageViews.size());

    for (size_t i = 0; i < imageViews.size(); i++) {
      std::array<VkImageView, 2> attachments = {imageViews[i],
                                                depthImageView};
    ...
    std::pair<VkRenderPass,std::vector<VkFramebuffer>> renderPassFramebuffersPair(renderPass,framebuffers);
    renderPassFramebuffersMap.insert(renderPassFramebuffersPair);
}
