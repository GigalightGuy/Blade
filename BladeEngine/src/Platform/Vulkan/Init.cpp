...
namespace BladeEngine::Vulkan 
{
	...
	static void CreateInstance(uint32_t apiVersion, uint32_t extensionsCount, const char *const *extensionNames) 
	{
	...
	static VkBool32 CheckDeviceExtensionSupport(VkPhysicalDevice device) 
	{
		...
		std::set<std::string> requiredExtensions(vkPhysicalDeviceExtensions.begin(), vkPhysicalDeviceExtensions.end());
	...
	static void CreateLogicalDevice(VkSurfaceKHR windowSurface) 
	{
  		...
  		if (vkCreateDevice(vkPhysicalDevice, &createInfo, nullptr, &vkDevice) != VK_SUCCESS) 
		{
  		...
  		vkGetDeviceQueue(vkDevice, GPQueueFamilyIndices.graphicsFamilyIndice.value(), 0, &graphicsQueue);
  		vkGetDeviceQueue(vkDevice, GPQueueFamilyIndices.presentFamilyIndice.value(), 0, &presentQueue);
	}
